...
class DualGaussianDiffusion(nn.Module):
    def __init__(
        self,
        denoise_fn,
        image_size,
    ...
    def ddim_pyramid_sample(
        ...
        for i in tqdm(reversed(range(0, ddim_timesteps)), desc='sampling loop time step', total=ddim_timesteps): 
            ...
            if pred_type == 'noise':
                ...
                pred_noise = self.denoise_fn(F.interpolate(x_in, sample_img.shape[2:]), sample_img,  noise_level)
    ...
    def p_losses_cs_pyramid(self, x_HR, x_SR, noise=None, different_t_in_one_batch=False, clip_noise=False, t_range=None, cs_on_shift=False,
        ...
        if frozen_denoise: 
            with torch.no_grad():
                if not self.conditional:
                    ...
                    model_output = self.denoise_fn(x_SR, x_noisy, continuous_sqrt_alpha_cumprod)
        else:
            if not self.conditional:
                model_output = self.denoise_fn(x_noisy, continuous_sqrt_alpha_cumprod)
            else:   
        ...
        self.x_recon_cs = self.color_fn(self.x_recon, continuous_sqrt_alpha_cumprod)
